    ...
    extends Controller
{
  ...
  function getMap($fac, $dark = false) {
    ...
    return view('tmu.tmu', ['coords_array' => json_encode($coords, JSON_NUMERIC_CHECK | JSON_PRESERVE_ZERO_FRACTION), 'coords_geoJSON' => $coords_geoJSON, 'min' => $min, 'max' => $max, 'default' => $default, 'colors' => json_encode($colors), 'fac' => $fac->id, 'facname' => $fac->name, 'dark' => $dark]);
  }
  ...
  function getMgtIndex($fac = null) {
      ...
      if ($fac == null) {
          if (\Auth::user()->facility == "ZHQ") { $fac = "ZAB"; }
          else { $fac = \Auth::user()->facility; }
      }
      if (!RoleHelper::isFacilitySeniorStaff(null, $fac) && !RoleHelper::hasRole(\Auth::user()->cid, $fac, "WM") && !RoleHelper::hasRole(\Auth::user()->cid, $fac, "FE")) abort(401);
    ...
    function postMgtCoords(Request $request, $ofac = null) {
        ...
        $tmufac->save();
        return redirect("/mgt/tmu/$fac")->with("success","Facility coordinates saved. <a href=\"/tmu/$fac\">Click here</a> to view map.");
    }
  ...
  function postMgtColors(Request $request, $ofac = null) {
    if (!\Auth::check()) abort(401);
      ...
      $appcolors = ['black','brown','blue','gray','green','lime','cyan','orange','red','purple','yellow','violet'];
      foreach($appcolors as $cl) {
          $colors->{$cl} = str_replace(" ","", $request->input($cl));
      }
      $colors->save();
      return redirect("/mgt/tmu" . (($ofac) ? "/$ofac" : $fac) . "/colors")->with("success","TMU Map Colors saved. <a href=\"/tmu/$ofac\">Click here</a> to view map.");
  }
  ...
  function getMgtMapping($fac,$id) {
      ...
      if ($id == 0) {
          ...
          if (!$map) abort(404);
          if ($map->parent_facility != $fac) abort(401);
      }
  ...
  function postMgtMapping(Request $request, $fac, $id) {
      $map = tmu_maps::find($id);
      if ($fac != $map->parent_facility) abort(401);
      if (!RoleHelper::isFacilitySeniorStaff(null, $fac) && !RoleHelper::hasRole(\Auth::user()->cid, $fac, "WM") && !RoleHelper::hasRole(\Auth::user()->cid, $fac, "FE")) abort(401);
