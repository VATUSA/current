...
class HelpdeskController
    ...
    public function getList(Request $request, $status) {
        ...
        if ($status == "mine") {
            ...
            $status = "My";
        } else if ($status == "myassigned") {
            $tickets = Ticket::where('assigned_to', Auth::user()->cid)->where('status', 'Open')->orderBy('created_at',
                'asc')->get();
            $status = "My Assigned";
        } else if ($status == "open") {
            $status = "Open";
    ...
    public function postList($status) {
        ...
        if ($status != "search") {
            return direct('/')->with("error", "Unknown status in post to ticket center");
        }
        ...
        if ($_POST['facility'] != "0") {
            if (isset($ticket)) {
                $ticket = $ticket->where('facility', $_POST['facility']);
            } else {
                $ticket = Ticket::where('facility', $_POST['facility']);
            }
        }
        if ($_POST['status'] != "0") {
            if (isset($ticket)) {
                $ticket = $ticket->where('status', $_POST['status']);
            } else {
                $ticket = Ticket::where('status', $_POST['status']);
            }
        }
    ...
    public function getTicketToggleStatus($id) {
        ...
        return redirect('/help/ticket/' . $ticket->id)->with("success",
    ...
    public function postTicket(Request $request, $id) {
        ...
        if ($request->input("tReply", null) == null) {
            return redirect("/help/ticket/$id")->with("error",
        ...
        if ($ticket->cid == Auth::user()->cid || AuthHelper::authACL()->canManageFacilityTickets($ticket->facility)) {
            ...
            if ($request->input("replyAndCloseSubmit")) {
                ...
                EmailHelper::sendSupportEmail($ticket->submitter->email, $ticket->id, "Ticket Closed",
                    "emails.help.closed", ["ticket" => $ticket, "closer" => Auth::user()->fullname()]);
            } else if ($request->input("replyAndOpenSubmit")) {
                $ticket->status = "Open";
                $history = new TicketHistory();
                $history->ticket_id = $ticket->id;
                $history->entry = Auth::user()->fullname() . " (" . Auth::user()->cid . ") opened the ticket.";
    ...
    public function postTicketAjax(Request $request, $id) {
        ...
        if (AuthHelper::authACL()->canManageFacilityTickets($ticket->facility)) {
            if ($request->input("facility")) {
                ...
                $history->ticket_id = $ticket->id;
                $history->entry =
                    Auth::user()->fullname() . " (" . Auth::user()->cid . ") assigned ticket to " . $ticket->facility .
                    " facility.";
                $history->save();
            }
            if (isset($_POST['assign'])) {
                if ($request->input("assign") == "0") {
                    ...
                    $history->ticket_id = $ticket->id;
                    $history->entry =
                        Auth::user()->fullname() . " (" . Auth::user()->cid . ") set ticket to unassigned.";
                    $history->save();
