...
class SMFHelper
{
    ...
    public static function setPermissions($cid)
    {
        ...
        if ($role) $grp = static::findFacilityStaff($user->facility);
        else $grp = static::findGroup("Members");

        if (RoleHelper::isVATUSAStaff($cid, true, true)) {
            $grp = static::findGroup("VATUSA Staff");
            $role = "";
            if (in_array($user->getPrimaryRole(), ['1','2','6'])) { $role = "Administrator"; }
        }
        if ($role) $addl = static::findGroup($role);
