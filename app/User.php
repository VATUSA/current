...
class User extends Model implements AuthenticatableContract, CanResetPasswordContract
{
    ...
    public function addToFacility($facility)
    {
        ...
        if ($this->rating >= Helper::ratingIntFromShort("I1"))
            SMFHelper::createPost(7262, 82, "User Addition: " . $this->fullname() . " (" . Helper::ratingShortFromInt($this->rating) . ") to " . $this->facility, "User " . $this->fullname() . " (" . $this->cid . "/" . Helper::ratingShortFromInt($this->rating) . ") was added to " . $this->facility . " and holds a higher rating.\n\nPlease check for demotion requirements.\n\n[url=https://www.vatusa.net/mgt/controller/" . $this->cid . "]Member Management[/url]");
    ...
    public function transferEligible(&$checks = null)
    {
        ...
        if ($this->flag_homecontroller) $checks['homecontroller'] = 1;
        else $checks['homecontroller'] = 0;
        if ($this->flag_needbasic == 0) $checks['needbasic'] = 1; // 1 = check passed
        ...
        if (!in_array($this->facility, ["ZAE","ZZN","ZHQ"])) {
            if (Transfers::where('cid', $this->cid)->where('to', 'NOT LIKE', 'ZAE')->where('to', 'NOT LIKE', 'ZZN')->where('status', 1)->count() == 1) {
                if (Carbon::createFromFormat('Y-m-d H:i:s', $this->facility_join)->diffInDays(new Carbon()) <= 30) $checks['initial'] = 1;
            } else {
                $checks['is_first'] = 0;
            }
        } else {
        ...
        $transfer = Transfers::where('cid', $this->cid)->where('status',1)->where('to', 'NOT LIKE', 'ZAE')->where('to','NOT LIKE', 'ZZN')->where('status', 1)->orderBy('created_at', 'DESC')->first();
        if (!$transfer) $checks['90days'] = 1;
        else {
        ...
        $promotion = Promotions::where('cid', $this->cid)->where("to", "<=",Helper::ratingIntFromShort("S3"))->where('created_at','>=',\DB::raw('DATE(NOW() - INTERVAL 90 DAY)'))->first();
        if ($promotion == null)
            $checks['promo'] = 1;
        else
            $checks['promo'] = 0;
...
/*        if ($this->facility == "ZAE" && !$this->flag_needbasic && !$this->selectionEligible() && !Transfers::where('cid', $this->cid)->where('status',0)->count())
    ...
    public function selectionEligible()
    {
        ...
        if ($this->facility == "ZAE" && !$this->flag_needbasic && Transfers::where('cid',$this->cid)->where('to', 'NOT LIKE', 'ZAE')->where('to', 'NOT LIKE', 'ZZN')->count() < 1)
//        if($this->facility == "ZAE")
            return true;
    }
